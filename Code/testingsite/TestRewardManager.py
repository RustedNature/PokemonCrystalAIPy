...
from Enums.MapBank import MapBank
from Enums.NeuborkiaMapNumber import NeuborkiaMapNumber
...
class RewardManager:
    def __init__(self):
        ...
        self.exploration_reward_counter = 0
    ...
    def get_reward(self, last_button_pressed, image : np.ndarray):
        self.reward = 0.0
    ...
    def exploration_reward(self):
        ...
        if  is_different_image:
            self.last_images.append(self.current_image)
            self.reward += 0.01
            print("Reward for exploring")

    
    # def is_image_different(self):
    #     new_image_tensor = F.to_tensor(self.current_image.transpose((1, 2, 0)))
    ...
    #     image_batch = torch.stack([F.to_tensor(img.transpose((1, 2, 0))) for img in self.last_images])
    ...
    #     is_different = not torch.any(mean_diff < 0.20)
